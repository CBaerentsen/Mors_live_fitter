...
class Fit(object):
    ...
    def fitter():
        start = time.time()
        data_fit = Fit.data[(Fit.f>=Fit.start) & (Fit.f<=Fit.end)]
        f_fit = Fit.f[(Fit.f>=Fit.start) & (Fit.f<=Fit.end)]
        
        scaling=np.min(data_fit)
        ...
        model.set_param_hint('r', value=0.08,min=0,max=1)
